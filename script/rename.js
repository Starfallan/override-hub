/**
 * 自用修改说明：
 * - 修改目的：修复原逻辑下因保留后缀（如倍率、IPLC 等）被错误分组导致序号不连续的问题；并调整序号显示位置,改至保留后缀前。
 * - 主要改动：
 *   1) 增加每条目字段 `._baseName`（基名）和 `._suffix`（保留字段），以便按基名统一编号。
 *   2) 修改 `jxh`：序号按 `._baseName` 统计并插入到保留字段之前，格式示例："香港 09 IEPL 0.1x"。
 *   3) 调整 `oneP`：基于 `._baseName` 判断仅一项时去掉序号。
 * - 变更日期：2025-11-20
 * - 原始仓库/参考：https://raw.githubusercontent.com/Keywos/rule/main/rename.js
 * - 备注：这些修改为本地增强，已尽量保持原脚本其它行为不变；如需还原，请以上游地址为准。
 */
...
function operator(pro) {
  ...
  pro.forEach((e) => {
    ...
    if (findKey?.[1]) {
      ...
      // 保留 baseName 与 suffix，方便后续按 baseName 分组编号
      const baseParts = [firstName, usflag, nNames, findKeyValue].filter((k) => k !== "");
      const suffixParts = [retainKey, ikey, ikeys].filter((k) => k !== "");
      e._baseName = baseParts.join(FGF);
      e._suffix = suffixParts.join(FGF);
      // 最终显示名称仍然保持与之前一致（base + 可选 suffix）
      keyover = keyover.concat(e._baseName, e._suffix).filter((k) => k !== "");
      e.name = keyover.join(FGF);
    } else {
      if (nm) {
        e._baseName = (FNAME + FGF + e.name).trim();
        e._suffix = "";
...
// prettier-ignore
function jxh(e) {
  // 按 _baseName 分组并按原始顺序递增序号，显示时保留 _suffix
  const counts = {};
  const out = e.map((item) => {
    const base = (item._baseName || item.name || "").trim();
    counts[base] = (counts[base] || 0) + 1;
    const seq = counts[base].toString().padStart(2, "0");
    // 将序号放在保留字段之前：<base><XHFGF><seq><FGF><suffix>
    const suffix = item._suffix ? FGF + item._suffix : "";
    const newName = `${base}${XHFGF}${seq}${suffix}`;
    return { ...item, name: newName };
  });
  e.splice(0, e.length, ...out);
  return e;
}
// prettier-ignore
