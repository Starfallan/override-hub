//额外修改 by Starfallen
// 添加了自动选择组：用以适配可能的Smart内核.智能选择最快的非高级节点（排除专线、IEPL、>1x倍率）
// 添加高倍率节点组：自动测速选择最快的高级节点
// 这样既满足了日常使用的自动选择需求，又为需要高速连接时提供了专门的高倍率组选项！
...
function buildBaseLists({ landing, lowCost, highCost, countryGroupNames }) {
...
function hasLowCost(config) {
    const lowCostRegex = /0\.[0-9]|低倍率|省流|大流量|实验性/i;
    return (config.proxies || []).some(proxy => lowCostRegex.test(proxy.name));
}

// 识别高倍率节点(即名称中包含大于1x的倍率,识别规则适配为JS兼容的正则表达式)
function hasHighCost(config) {
    const highCostRegex = /((\s|-)(([1-9](\.\d+)?)(×|x|X))|专线|专用|高级|急速|高倍率|IEPL|IPLC|AIA|CTM|CC|iepl|iplc|aia|ctm|cc)/i;
    return (config.proxies || []).some(proxy => highCostRegex.test(proxy.name));
}
...
function buildProxyGroups({
    ...
    return [
        ...
        },
        {
            ...
            "url": "https://cp.cloudflare.com/generate_204",
            "include-all": true,
            "exclude-filter": "(( \\s|-)((?:[2-9]\\d*(?:\\.\\d+)?|1\\.\\d+)(×|x|X))|专线|专用|高级|急速|高倍率|IEPL|IPLC|AIA|CTM|CC|iepl|iplc|aia|ctm|cc)",
            "interval": 60,
            "tolerance": 20,
            "lazy": false
        },
        ...
        (highCost) ? {
            ...
            "url": "https://cp.cloudflare.com/generate_204",
            "include-all": true,
            "filter": "(( \\s|-)((?:[2-9]\\d*(?:\\.\\d+)?|1\\.\\d+)(×|x|X))|专线|专用|高级|急速|高倍率|IEPL|IPLC|AIA|CTM|CC|iepl|iplc|aia|ctm|cc)"
        } : null,
        ...countryProxyGroups
...
function main(config) {
    ...
    const highCost = hasHighCost(resultConfig);
    ...
    const {
        ...
        defaultSelector,
        defaultFallback
    } = buildBaseLists({ landing, lowCost, highCost, countryGroupNames });
